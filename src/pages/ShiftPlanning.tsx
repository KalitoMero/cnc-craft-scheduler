...
import { Plus, Trash2, UserPlus, CalendarDays, AlertCircle, Users } from "lucide-react";
...
export default function ShiftPlanning() {
  ...
  const loadData = async () => {
    try {
      const [employeesData, machinesData, shiftsData, assignmentsData, sickDaysData] = await Promise.all([
        api.getEmployees(),
        api.getMachines(),
  ...
  const handleSaveEmployee = async () => {
    ...
    try {
      if (editingEmployee) {
        await api.updateEmployee(editingEmployee.id, { name: employeeName.trim() });
        toast({ title: "Erfolg", description: "Mitarbeiter aktualisiert." });
      } else {
        await api.createEmployee({ name: employeeName.trim() });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setEditingEmployee(null); setEmployeeName(""); setShowEmployeeDialog(true); }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="secondary" className="text-xs">
                      {getEmployeeAssignedShifts(emp.id).length} Schichten
                    </Badge>
                  </div>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>
              {selectedEmployee ? `Schichten für ${selectedEmployee.name}` : "Mitarbeiter auswählen"}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {!selectedEmployee ? (
